...
> **区間の目印が無いブランチは、`main` の区間で代用します。** この仕組みより前から
> 出ている PR には目印がありません。そこでエラーにすると、ルールに触っていない
> 既存の PR がプレビューを出せなくなるためです。目印の数が合わない（書きかけ・
> 消し忘れ）場合は代用せず拒否します — どちらのつもりか分からないものを、黙って
> `main` のルールで動かすと意図と違うものを見せることになるためです。
